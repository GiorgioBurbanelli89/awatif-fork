...
export function deformCpp(
  ...
  // 1- Allocate data
  // Nodes
  const nodesPtr = allocate(nodes.flat(), Float64Array, mod.HEAPF64 as any);
  gc.push(nodesPtr);

  // Elements
  const elementIndices = elements.flat();
  const elementsPtr = allocate(elementIndices, Uint32Array, mod.HEAPU32 as any);
  gc.push(elementsPtr);
  const elementSizes = elements.map((e) => e.length);
  const elementSizesPtz = allocate(elementSizes, Uint32Array, mod.HEAPU32 as any);
  ...
  const supportValues = nodeInputs.supports
    ? Array.from(nodeInputs.supports.values())
        ...
        .map((b) => (b ? 1 : 0))
    : [];
  const supportKeysPtr = allocate(supportKeys, Uint32Array, mod.HEAPU32 as any);
  gc.push(supportKeysPtr);
  const supportValuesPtr = allocate(supportValues, Uint8Array, mod.HEAPU8 as any);
  ...
  const loadValues = nodeInputs.loads
    ...
    : [];
  const loadKeysPtr = allocate(loadKeys, Uint32Array, mod.HEAPU32 as any);
  gc.push(loadKeysPtr);
  const loadValuesPtr = allocate(loadValues, Float64Array, mod.HEAPF64 as any);
  gc.push(loadValuesPtr);

  // NodeInputs.springs
  const springKeys = nodeInputs.springs
    ? Array.from(nodeInputs.springs.keys())
    : [];
  const springValues = nodeInputs.springs
    ? Array.from(nodeInputs.springs.values()).flat()
    : [];
  const springKeysPtr = allocate(springKeys, Uint32Array, mod.HEAPU32 as any);
  gc.push(springKeysPtr);
  const springValuesPtr = allocate(springValues, Float64Array, mod.HEAPF64 as any);
  gc.push(springValuesPtr);
  ...
  const processElementInput = (inputMap: Map<number, number> | undefined) => {
    ...
    const keysPtr = allocate(keys, Uint32Array, mod.HEAPU32 as any);
    gc.push(keysPtr);
    const valuesPtr = allocate(values, Float64Array, mod.HEAPF64 as any);
    gc.push(valuesPtr);
