...
Eigen::MatrixXd getTransformationMatrixQuad4(const std::vector<Node> &nodes);
...
Eigen::MatrixXd getTransformationMatrix(const std::vector<Node> &nodes)
{
    ...
    if (nodes.size() == 4)
    {
        return getTransformationMatrixQuad4(nodes);
    }
...
// Transformation matrix for 4-node Quad4 shell elements
Eigen::MatrixXd getTransformationMatrixQuad4(const std::vector<Node> &nodes)
{
    const int nodesCount = 4;
    const int dimensions = 3;
    const int dof = 6;
    const int totalDof = nodesCount * dof; // 24

    // Build local coordinate system from the element plane
    // Use first 3 nodes to define the plane (similar to triangle approach)
    ...
    // Calculate two edge vectors for the plane
    Eigen::Vector3d v1 = n1 - n0;  // Edge 1-0
    Eigen::Vector3d v2 = n3 - n0;  // Edge 3-0 (diagonal direction)

    // Normal vector (local Z axis)
    Eigen::Vector3d normalVector = v1.cross(v2);
    double length = normalVector.norm();

    if (length < 1e-12)
    {
        // Try alternative: use edges 1-2 and 2-3
        v1 = n2 - n1;
        v2 = n3 - n2;
        normalVector = v1.cross(v2);
        length = normalVector.norm();

        if (length < 1e-12)
        {
            std::cerr << "Warning: Degenerate Quad4: nodes are coplanar or coincident. Returning zero matrix." << std::endl;
            return Eigen::MatrixXd::Zero(totalDof, totalDof);
        }
    }
    ...
    // Local X axis: project global X onto the element plane
    Eigen::Matrix3d globalIdentityMatrix = Eigen::Matrix3d::Identity();
    double dotProductWithGlobalX = normalVector.x();
    Eigen::Vector3d localX;
    ...
    if (std::abs(dotProductWithGlobalX) > 1.0 - 1e-10)
    {
        // Normal is nearly parallel to global X, use global Z instead
        double dotProductWithGlobalZ = normalVector.z();
        localX = globalIdentityMatrix.col(2) - dotProductWithGlobalZ * normalVector;
    }
    else
    ...
    if (length < 1e-12)
    {
        std::cerr << "Warning: Degenerate local X-axis detected in Quad4. Returning zero matrix." << std::endl;
        return Eigen::MatrixXd::Zero(totalDof, totalDof);
    }
    localX.normalize();

    // Local Y axis: perpendicular to both Z and X
    Eigen::Vector3d localY = localZ.cross(localX);
    length = localY.norm();
    if (length < 1e-12)
    {
        std::cerr << "Warning: Degenerate local Y-axis detected in Quad4. Returning zero matrix." << std::endl;
        return Eigen::MatrixXd::Zero(totalDof, totalDof);
    }
    localY.normalize();

    // Build 3x3 rotation matrix (local to global)
    Eigen::Matrix3d transformationMatrixLocal;
    transformationMatrixLocal.row(0) = localX.transpose();
    transformationMatrixLocal.row(1) = localY.transpose();
    transformationMatrixLocal.row(2) = localZ.transpose();
    ...
    for (int nodeIdx = 0; nodeIdx < nodesCount; ++nodeIdx)
    {
        ...
        // Apply 3x3 rotation to both translational and rotational DOFs
        for (int i = 0; i < dimensions; ++i)
        {
            for (int j = 0; j < dimensions; ++j)
            {
