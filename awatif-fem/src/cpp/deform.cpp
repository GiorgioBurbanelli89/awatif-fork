...
extern "C"
{
    void deform(
        ...
        // Springs (nodal spring stiffnesses)
        int *spring_keys_ptr, double *spring_values_ptr, int num_springs, // Map<nodeIdx, [Kx, Ky, Kz, Krx, Kry, Krz]>
    ...
    )
    {
        ...
        // Parse NodeInputs (supports, loads, and springs)
        ...
        nodeInputs.springs = parseMapVecFromFlat(spring_keys_ptr, spring_values_ptr, num_springs, 6);
        ...
        // Add spring stiffnesses to K_global diagonal
        for (const auto &pair : nodeInputs.springs)
        {
            int nodeIndex = pair.first;
            const auto &springK = pair.second;
            if (springK.size() == 6)
            {
                for (int i = 0; i < 6; ++i)
                {
                    if (springK[i] > 0)
                    {
                        int globalDof = nodeIndex * 6 + i;
                        K_global.coeffRef(globalDof, globalDof) += springK[i];
                    }
                }
            }
        }
        ...
        for (int i = 0; i < num_nodes; ++i)
        {
            ...
            if (hasSupport)
            {
                ...
                for (int j = 0; j < 6; ++j)
                {
                    // FEM reaction = K*U - F at supported DOFs
                    // This accounts for external loads applied directly at support nodes
