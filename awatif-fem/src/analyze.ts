...
export function analyze(
  ...
  if (!nodes || nodes.length === 0 || !elements || elements.length === 0) {
    return {
      normals: new Map(),
      shearsY: new Map(),
      shearsZ: new Map(),
  ...
  // Skip if no deformations data
  if (!deformOutputs?.deformations || deformOutputs.deformations.size === 0) {
    return analyzeOutputs;
  }

  elements.forEach((e, i) => {
    // Skip if any node index is invalid
    if (e.some(idx => idx < 0 || idx >= nodes.length || !nodes[idx])) return;

    const elmNodes = e.map((idx) => nodes[idx]);

    // Skip if any element node is undefined
    if (elmNodes.some(n => !n)) return;

    // Get global deformations for this element's nodes
    ...
    // Skip if no valid deformations
    if (dxGlobal.some(v => v === undefined || v === null || isNaN(v))) return;

    const T = getTransformationMatrix(elmNodes);

    // Skip if transformation matrix is invalid
    if (!T || (Array.isArray(T) && T.some(row => Array.isArray(row) && row.some(v => v === undefined || isNaN(v))))) return;
