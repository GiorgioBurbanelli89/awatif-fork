...
let __tla = (async () => {
  ...
  ln = function(W, d, O) {
    ...
    if (d && d.length > 0) {
      ...
      P = Promise.allSettled(d.map((U) => {
        if (U = cn(U, O), U in Nr) return;
        Nr[U] = true;
        const G = U.endsWith(".css"), ke = G ? '[rel="stylesheet"]' : "";
        if (!!O) for (let K = $.length - 1; K >= 0; K--) {
          const q = $[K];
  ...
  async function dn(Y = {}) {
    ...
    var d = Y, O = !!globalThis.window, P = !!globalThis.WorkerGlobalScope, j = ((_b = (_a = globalThis.process) == null ? void 0 : _a.versions) == null ? void 0 : _b.node) && ((_c = globalThis.process) == null ? void 0 : _c.type) != "renderer", $ = !O && !j && !P;
    if (j) {
    ...
    var Z;
    function Ye() {
      return d.locateFile ? ke("deform.wasm") : new URL("" + new URL("deform-DkAibFYt.wasm", import.meta.url).href, import.meta.url).href;
    }
    function ze(e) {
    ...
    var je = (e) => yr(e), cr = (e) => {
      ...
      var v = (_) => {
        var S = _ >= 0 ? "-" : "+", M = Math.abs(_), H = String(Math.floor(M / 60)).padStart(2, "0"), x = String(M % 60).padStart(2, "0");
        return `UTC${S}${H}${x}`;
      }, h = v(c), m = v(l);
...
`));
    ...
    }, _e = 0, Me = null, ye = {}, le = null, lt = (e) => {
      var _a2;
      if (_e--, (_a2 = d.monitorRunDependencies) == null ? void 0 : _a2.call(d, _e), f(e, "removeRunDependency requires an ID"), f(ye[e]), delete ye[e], _e == 0 && (le !== null && (clearInterval(le), le = null), Me)) {
        var r = Me;
        Me = null, r();
      }
    }, dt = (e) => {
      ...
      createLazyFile(e, r, t, n, i) {
        class a {
          ...
          cacheLength() {
            ...
            var _ = Number(m.getResponseHeader("Content-length")), S, M = (S = m.getResponseHeader("Accept-Ranges")) && S === "bytes", H = (S = m.getResponseHeader("Content-Encoding")) && S === "gzip", x = 1024 * 1024;
            M || (x = _);
            var X = (Q, Se) => {
        ...
        function v(h, m, _, S, M) {
          var H = h.node.contents;
          if (M >= H.length) return 0;
          var x = Math.min(H.length - M, S);
          if (f(x >= 0), H.slice) for (var X = 0; X < x; X++) m[_ + X] = H[M + X];
          else for (var X = 0; X < x; X++) m[_ + X] = H.get(M + X);
          return x;
        }
        return u.read = (h, m, _, S, M) => (o.forceLoadFile(l), v(h, m, _, S, M)), u.mmap = (h, m, _, S, M) => {
          o.forceLoadFile(l);
          var H = fr();
    ...
    function on(e, r, t, n, i, a, s, c, l, u, v, h, m, _, S, M) {
      var H = T();
      try {
        b(e)(r, t, n, i, a, s, c, l, u, v, h, m, _, S, M);
      } catch (x) {
  ...
  fn = function(Y, W, d, O) {
    ...
    }, E = V(O.elasticities), fe = V(O.elasticitiesOrthogonal), se = V(O.areas), N = V(O.momentsOfInertiaZ), Fe = V(O.momentsOfInertiaY), Te = V(O.shearModuli), be = V(O.torsionalConstants), ve = V(O.thicknesses), he = V(O.poissonsRatios), pe = R._malloc(4);
    P.push(pe);
    const B = R._malloc(4);
    P.push(B);
  ...
  function J(Y, W, d) {
    const O = new W(Y), P = R._malloc(O.length * O.BYTES_PER_ELEMENT);
    return d.set(O, P / O.BYTES_PER_ELEMENT), P;
  }
})();
export {
  ln as _,
