...
export function elements(
  ...
  van.derive(() => {
    ...
    if (!derivedNodes.val || derivedNodes.val.length === 0) return;
    if (!mesh.elements?.val || mesh.elements.val.length === 0) return;

    const buffer = mesh.elements?.val
      .map((e) =>
        elementToEdges(e)
          .map((edge) => {
            const n1 = derivedNodes.val[edge[0]];
            const n2 = derivedNodes.val[edge[1]];
            if (!n1 || !n2) return [0, 0, 0, 0, 0, 0];
            return [...n1, ...n2];
          })
          .flat()
      )
